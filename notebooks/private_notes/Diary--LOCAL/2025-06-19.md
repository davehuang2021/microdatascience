...
- 昨天11点做完PPT，今天早上五点多发出。不紧不慢，把握好节奏。
- 昨天搞定excel读取功能，对处理复杂文档还是必须的。昨天为了尽快找到数据也是拼了
- 昨天：delta 值的使用有时可以让数据很好看，多尝试着这个技巧
- 搞定了apple mail使用工作邮箱，终于不用管网易的狗屁客户端了
- 代码优化用了一上午，最终还是没有数据库效率高呀。。。
